exports.handler = async (event) => {
    try {
        ...
        //Sender Validation
        ///If the sender is a teacher or an admin then let them create a user
        if (senderUserData.accessLevel == "teacher" || senderUserData.accessLevel == "admin") {
            ...
            } else if (!(/[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|[1-2][0-9]|3[01])/.test(newUserData.DOB))) {
            ...
            var numOfEmail = await isEmailUnique(newUserData.email);
            numOfEmail = numOfEmail.records[0][0].longValue;

            if (numOfEmail != 0) {
                throw ("You cannot create a user that already exists");
            }
...
async function isEmailUnique(newEmail) {
    //Create User
    ...
    var sqlParams = {
        ...
        sql: 'SELECT COUNT(*) FROM users WHERE email="' + newEmail + '" AND AND user_active=1;',
