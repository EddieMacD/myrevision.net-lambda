exports.handler = async (event) => {
    try {
        ...
        assignmentData.filters = JSON.stringify(params.assignmentData.filters);
        ...
        if (assignmentData.name.length > 31 || assignmentData.name.length < 0) {
        ...
        } else if (assignmentData.info.length > 255 || assignmentData.name.length < 0) {
            ...
            ///Format check the deadline
            throw ("This date is invalid");
        } else if (assignmentData.filters.length > 1000 || assignmentData.filters.length < 0) {
            ///Range check the filters
            throw ("These filters are invalid");
        }
        ...
        try {
            ...
            var dbData = await createAssignment(assignmentData);
        } catch (e) {
            console.log(e);
            throw ("Error creating assignment");
        }

        ///Create notifications about the assignment being created
        await createNotifications(dbData.generatedFields[0].longValue);
...
///assignmentData: an object containing all of the relevant parameters for creating an assignment
async function createAssignment(assignmentData) {
    ...
    ///This SQL query creates a new row in assignments with a name, description, deadline, filters_json, class_id
    var sqlParams = {
        ...
        sql: 'INSERT INTO assignments (name, description, deadline, filters_json, class_id) VALUES ("' + assignmentData.name + '", "'
            + assignmentData.info + '", "' + assignmentData.deadline + '", ' + JSON.stringify(assignmentData.filters) + ', ' + assignmentData.classID + ');',
