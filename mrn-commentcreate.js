exports.handler = async (event) => {
    try {
        ...
        var comment = event.queryStringParameters.comment;
        ...
        try {
            ///Recall the ID of the correct instance of question history
            historyID = await getHistoryID(senderUserEmail, filePath, fileName);

            ///Recall the ID of the user
            var userID = await getUserID(senderUserEmail);

            ///Create a comment on the database
            await createComment(comment, userID, recieverID, historyID);
        } catch (e) {
            console.log(e);
            throw ("error creating comment");
        }

        ///Create a notification for the correct teacher that a comment has been sent
        responseBody = await sendNotification(historyID, recieverID, filePath, fileName, senderUserEmail, comment);
...
///userID: the ID of the user that sent the comment
///recieverID: the ID of the teacher that the comment was sent to
///historyID: the ID of this specific instance of question history
async function createComment(comment, userID, recieverID, historyID) {
    //Create Comment
    ///An object containing all of the necessary parameters to query the database. Includes credentials, the database cluster's ARN, the database's name and the SQL query
    ///The SQL statement 
    var sqlParams = {
        secretArn: 'arn:aws:secretsmanager:eu-west-2:293120934689:secret:rds-db-credentials/cluster-V4YUHCBNOQZWAQNYBYOORBOKP4/admin-hQLZYp',
        resourceArn: 'arn:aws:rds:eu-west-2:293120934689:cluster:mrn-database-cluster',
        sql: 'INSERT INTO question_feedback (timestamp, comment, sender_id, reciever_id, question_history_id) VALUES ("' + getTimestamp() + '", "' + comment + '", ' + userID + ', ' + recieverID + ', ' + historyID + ');',
        database: 'mrn_database',
        includeResultMetadata: false
    };
...
//Sends a notification to the relevant teacher that a comment was created
///historyID: the ID of this specific instance of question history
///recieverID: the ID of the teacher that the comment was sent to
///filePath: the file path of the relevant question
///fileName: the file name of the relevan question
///senderUserEmail: the email of the user that sent the comment
///comment: the text of the comment
async function sendNotification(historyID, recieverID, filePath, fileName, senderUserEmail, comment) {
    //Get Details
