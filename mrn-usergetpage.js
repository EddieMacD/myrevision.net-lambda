...
async function getUserPage(school, offset, amount) {
    ...
    var sqlParams = {
        ...
        sql: 'SELECT U.first_name, U.last_name, U.email, U.dob, U.access_level FROM users U INNER JOIN schools S ON U.school_id = S.school_id WHERE S.name="'
            + school + '" AND U.access_level <> "admin" AND U.user_active <> 0 ORDER BY U.access_level DESC, U.first_name, U.last_name ASC LIMIT ' + offset + ', ' + amount + ' ;',
...
async function getSchoolCount(school) {
    ...
    var sqlParams = {
        secretArn: 'arn:aws:secretsmanager:eu-west-2:293120934689:secret:rds-db-credentials/cluster-V4YUHCBNOQZWAQNYBYOORBOKP4/admin-hQLZYp',
        resourceArn: 'arn:aws:rds:eu-west-2:293120934689:cluster:mrn-database-cluster',
        sql: 'SELECT COUNT(*) FROM users U INNER JOIN schools S ON U.school_id = S.school_id  WHERE S.name="' + school + '" AND U.access_level <> "admin" AND U.user_active <> 0 GROUP BY S.school_id;',
        database: 'mrn_database',
        includeResultMetadata: false
    };
