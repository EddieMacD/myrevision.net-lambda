exports.handler = async (event) => {
    try { 
        //Data Extraction
        ///The ID of the class that a user is being added to
        ...
        ///The ID of the user that is being added to the class
        var userID = event.queryStringParameters.userID;


        //Sender Validation
        ///If the sender is a teacher or an admin then let them link a user to a class
        ...
        { 
            ...
            ///Encapsulated database failure and basic error message to prevent database's existance from being public. Helps prevent SQL injection
                try {
                    ///Links a user to the class
                    await linkUserClass(userID, classID);
                } catch (e) {
...
//Links a user to a class by creating an entry in the link table
///userID: the ID of the user that is being linked to a class
///classID: the ID of the class that the user being linked to
async function linkUserClass (userID, classID) {
    //Create Link
    ///An object containing all of the necessary parameters to query the database. Includes credentials, the database cluster's ARN, the database's name and the SQL query
    ///The SQL statement inserts a user ID and class ID into the user class link table
    var sqlParams = {
        secretArn: 'arn:aws:secretsmanager:eu-west-2:293120934689:secret:rds-db-credentials/cluster-V4YUHCBNOQZWAQNYBYOORBOKP4/admin-hQLZYp',
        resourceArn: 'arn:aws:rds:eu-west-2:293120934689:cluster:mrn-database-cluster',
        sql: 'INSERT INTO user_class_link (user_id, class_id) VALUES (' + userID + ', ' + classID + ');',
        database: 'mrn_database',
        includeResultMetadata: false
    };
